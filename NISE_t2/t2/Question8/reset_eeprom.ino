...
void setup() {
  ...
  {
  packetHandler->write1ByteTxRx(portHandler, id, 3, DXL_ID, &dxl_error);delay(1); // ID
  packetHandler->write1ByteTxRx(portHandler, id, 4, 1, &dxl_error);delay(1); // Baud rate
  packetHandler->write1ByteTxRx(portHandler, id, 5, 250, &dxl_error);delay(1); // Return delay time
  packetHandler->write2ByteTxRx(portHandler, id, 6, 0, &dxl_error);delay(1); // Clockwise angle limit
  packetHandler->write2ByteTxRx(portHandler, id, 8, 1023, &dxl_error);delay(1); // Counter-clockwise angle limit
  packetHandler->write1ByteTxRx(portHandler, id, 11, 70, &dxl_error);delay(1); // MAximal Internal temperature limit
  packetHandler->write1ByteTxRx(portHandler, id, 12, 60, &dxl_error);delay(1); // min input voltage linit
  packetHandler->write1ByteTxRx(portHandler, id, 13, 140, &dxl_error);delay(1); // max input voltage limit
  packetHandler->write2ByteTxRx(portHandler, id, 14, 1023, &dxl_error);delay(1); // max torque
