...
void check_sensors(struct Neuron* neuron_pointer, int neuron_index)
{
  ...
  if (sensor0 < FL_SENSOR_PIN_THR) { // OBSTACLE ON THE LEFT
    neuron_pointer -> y = 1.0; // if left sensor is triggered, turn head right
    reduce_cpg_amplitudes_left();
  }
  else if (sensor1 < FR_SENSOR_PIN_THR) { // OBSTACLE ON  THE RIGHT
    neuron_pointer -> y = 0.0; // if right sensor is triggered, turn head left
