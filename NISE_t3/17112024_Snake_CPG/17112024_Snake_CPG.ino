  ...
  for (int i = 0; i < NUM_MOTORS; i++)
    {
      ...
      total_signal = left.y - right.y;


      if (total_signal > 0)
      {
        motors[i].goalPosition = mapFloat(total_signal, 0, 1, 512, 0); // 512 is a neutral position, 0 is leftmost position

      }

      else 
      {
        motors[i].goalPosition = mapFloat(abs(total_signal), 0, 1, 512, 1023); // 512 is a neutral position, 1023 is rightmost position
      }
